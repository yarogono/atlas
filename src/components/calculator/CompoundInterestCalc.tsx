    ...
    <div className="not-prose w-full select-none transition-all duration-300">
        ...
        <div className="p-6 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 bg-white dark:bg-slate-900">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="원금 입력"
                  className="w-full py-2.5 pl-4 pr-10 font-bold text-sm text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200/80 dark:border-slate-700 focus:border-blue-500 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-blue-500/10 outline-none transition-all"
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 text-xs font-bold">원</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="예: 20"
                  className="w-full py-2.5 px-4 font-bold text-sm text-slate-800 dark:text-slate-100 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200/80 dark:border-slate-700 focus:border-blue-500 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:ring-blue-500/10 outline-none transition-all"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-400 dark:text-slate-500 mb-1.5">
                ...
                <select
                  ...
                  className="w-full py-2.5 px-3 font-bold text-sm text-slate-700 dark:text-slate-200 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200/80 dark:border-slate-700 outline-none focus:ring-2 focus:ring-blue-500/10 transition-all cursor-pointer"
                >
          ...
          <div className="lg:col-span-7 flex flex-col justify-between space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {/* 최종 자산 */}
              <div className="bg-blue-50/40 dark:bg-blue-950/10 border border-blue-100/50 dark:border-blue-900/30 p-4 rounded-2xl shadow-sm text-center">
                <span className="text-[10px] font-bold text-blue-500 dark:text-blue-400 uppercase tracking-widest block mb-1">최종 자산 (세후)</span>
                <span className="text-lg md:text-xl font-black text-slate-800 dark:text-slate-100 tracking-tight">
                  {Math.round(calculationResults.finalAsset).toLocaleString()}원
                </span>
              </div>

              {/* 총 투자 원금 */}
              <div className="bg-slate-50/50 dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800/80 p-4 rounded-2xl shadow-sm text-center">
                <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest block mb-1">총 투자 원금</span>
                <span className="text-lg md:text-xl font-black text-slate-800 dark:text-slate-100 tracking-tight">
              ...
              {/* 누적 순수익 */}
              <div className="bg-emerald-50/40 dark:bg-emerald-950/10 border border-emerald-100/50 dark:border-emerald-900/30 p-4 rounded-2xl shadow-sm text-center">
                <span className="text-[10px] font-bold text-emerald-500 dark:text-emerald-400 uppercase tracking-widest block mb-1">누적 순수익 (세후)</span>
                <span className="text-lg md:text-xl font-black text-emerald-600 dark:text-emerald-400 tracking-tight block">
                  {Math.round(calculationResults.finalNetProfit).toLocaleString()}원
                </span>
                <span className="text-[10px] font-bold text-emerald-500/80 bg-emerald-50 dark:bg-emerald-950/50 px-2 py-0.5 rounded-full inline-block mt-1">
            ...
            <div className="border border-slate-150 dark:border-slate-800 rounded-3xl p-4 bg-slate-50/30 dark:bg-slate-950/10 relative">
              <div className="flex justify-between items-center mb-3">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">복리 시각화 그래프</span>
              ...
              <div className="relative w-full overflow-hidden">
                ...
                {hoverCoords && currentHoverItem && (
                  <div
                    className="absolute bg-slate-900/95 text-white p-3 rounded-xl border border-slate-800 shadow-2xl text-[10px] font-sans leading-normal pointer-events-none transition-all duration-75"
                    style={{
                  ...
                  >
                    <div className="font-bold text-center border-b border-white/10 pb-1 mb-1 text-emerald-400">
                    ...
                    <div className="flex justify-between gap-4 text-emerald-300 font-bold border-t border-white/5 pt-1 mt-1">
                      <span>순수익금:</span>
      ...
      <div className="w-full bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-lg p-6 mb-12">
        <h2 className="text-base font-black text-slate-800 dark:text-slate-100 mb-4 flex items-center gap-2">
          ...
          <span className="text-[10px] text-slate-400 bg-slate-50 dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-100 dark:border-slate-800">
            총 {calculationResults.pVal}개 회차
          </span>
        </h2>

        <div className="max-h-[300px] overflow-y-auto border border-slate-150 dark:border-slate-800 rounded-2xl scrollbar-thin">
          <table className="w-full border-collapse text-xs text-left">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-950/60 text-slate-500 dark:text-slate-400 font-black border-b border-slate-150 dark:border-slate-800">
                <th className="py-3 px-4 text-left w-16">회차</th>
                <th className="py-3 px-4 text-right">회차별 이자</th>
                <th className="py-3 px-4 text-right">누적 원금</th>
                <th className="py-3 px-4 text-right">최종 자산 (세전)</th>
                <th className="py-3 px-4 text-right text-emerald-600 dark:text-emerald-400">최종 자산 (세후)</th>
                <th className="py-3 px-4 text-right text-indigo-500">누적 순수익</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-semibold text-slate-700 dark:text-slate-350">
              {calculationResults.history.map((item) => (
                ...
                >
                  <td className="py-3 px-4 font-black">{item.cycle === 0 ? '시작' : `${item.cycle}회`}</td>
                  <td className="py-3 px-4 text-right">
                    {item.cycle === 0 ? '-' : `${Math.round(item.interest).toLocaleString()}원`}
                  </td>
                  <td className="py-3 px-4 text-right">{item.cumulativeDeposit}원</td>
                  <td className="py-3 px-4 text-right">{Math.round(item.assetBeforeTax).toLocaleString()}원</td>
                  <td className="py-3 px-4 text-right font-bold text-emerald-600 dark:text-emerald-400">
                    {Math.round(item.assetAfterTax).toLocaleString()}원
                  </td>
                  <td className="py-3 px-4 text-right font-bold text-indigo-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 text-slate-800 dark:text-slate-200">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-md transition-transform duration-200 hover:scale-[1.01] hover:shadow-lg">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-base font-extrabold m-0 tracking-tight">복리의 개념</h2>
          </div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
            시간이 지날수록 발생한 이자가 다시 원금으로 합산되어 새로운 이자를 창출하기 때문에, 투자 기간이 늘어날수록 자산 증가 속도가 비약적인 기하급수적 곡선을 그리기 시작합니다. 세계적인 물리학자 알베르트 아인슈타인은 이를 두고 **"복리는 인류 최대의 수학적 발견이자 세계 제8대 불가사의"**라고 칭송한 바 있습니다.
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-md transition-transform duration-200 hover:scale-[1.01] hover:shadow-lg">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-base font-extrabold m-0 tracking-tight">마법의 복리 72 법칙</h2>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mb-4">
            투자의 세계에서 내가 투입한 자산이 정확히 **두 배(100% 수익률)**가 되기까지 걸리는 시간(연수)을 구하는 간단하고 획기적인 법칙이 바로 **'72 법칙'**입니다.
          </p>
          <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/60 font-mono text-[11px] text-slate-600 dark:text-slate-350 text-center mb-4 font-bold">
            두 배 도달 기간 = 72 / 연간 수익률 (%)
          </div>
          <p className="text-xs text-slate-650 dark:text-slate-400 leading-relaxed">
            예를 들어 매년 꾸준히 **8%**의 연복리 수익을 실현하는 금융 상품이 있다면, 자산이 두 배가 되기까지는 <code className="bg-slate-100 dark:bg-slate-850 px-1 py-0.5 rounded text-[10px]">72 / 8 = 9</code>년의 세월이 소요됩니다. 
            만약 복리 이율을 높여 연 수익률 **12%**를 보장받는다면, 원금이 두 배가 되는 시간은 단 **6년**으로 단축됩니다.
          </p>
        </div>
        ...
        <div className="bg-emerald-50/30 dark:bg-emerald-950/10 border border-emerald-250/40 dark:border-emerald-900/20 rounded-3xl p-6 shadow-md transition-transform duration-200 hover:scale-[1.01] hover:shadow-lg">
          ...
          <p className="text-xs text-slate-650 dark:text-slate-400 leading-relaxed mb-4">
            목돈을 한 번에 넣어두는 거치식 복리도 훌륭하지만, 일상 속에서 **매 주기마다 추가 납입을 곁들이는 적립식 복리 투자**는 가장 보편적이고 안전한 자산 증식 설계입니다.
          </p>
          <p className="text-xs text-slate-650 dark:text-slate-400 leading-relaxed">
            주기적인 분할 매수(추가 납입) 효과를 통해 자산 가격 변동의 리스크를 평균화하는 **달러 비용 평균법(Dollar Cost Averaging)**을 극대화할 수 있으며, 이자에 이자가 불어나는 속도에 적립 원금의 가중치가 더해져 훨씬 강력한 마법이 실현됩니다.<br/><br/>
